...
class Surface {
    ...
    static ArrayList<ConcurrentLinkedQueue<Request>> requestQueue = new ArrayList<>();
    static ArrayList<Boolean> isThreadSending = new ArrayList<>();
    static ArrayList<Boolean> isFinished = new ArrayList<>();
    static HashMap<Vertex, Long> vertexThreads = new HashMap<>();
    static HashMap<Long, Integer> activeThreads = new HashMap<>();
    ...
    class Request {
        ...
        //
        public void relax(ArrayList<LinkedHashSet<Vertex>> buckets) throws Coordinator.KilledException {
            Vertex o = e.other(v);
    ...
    public void DeltaSolve() throws Coordinator.KilledException {
      System.out.println("start delta solve");
      numBuckets = 2 * degree;
      ...
      DeltaHelperThread[] DHThreads = new DeltaHelperThread[numThreads];
      for (int i=0; i<numThreads; i++) {
        DHThreads[i] = new DeltaHelperThread(numBuckets, i, barrier,  numThreads);
        isThreadSending.add(false);
        isFinished.add(false);
        requestQueue.add(new ConcurrentLinkedQueue<Request>());
      }
      for(int i=0; i<n; i++){
        vertexThreads.put(vertices[i], DHThreads[i%numThreads].getId());
      }
      for(int i=0; i<numThreads; i++){
        activeThreads.put(DHThreads[i].getId(),i);
        DHThreads[i].start();
      }
      for(int i=0; i<numThreads; i++){
        try{
      ...
      System.out.println("end delta solve");
    }

    class DeltaHelperThread extends Thread {
      private int index;
      private int thread_number;
      private int numBucketss;
      private int numThreads;
      private ArrayList<LinkedHashSet<Vertex>> bucketss;
      private CyclicBarrier barrier;
      private Boolean willLoop;

      public DeltaHelperThread(int numBuckets, int thread_number, CyclicBarrier barrier, int numThreads){
        this.index = 0;
        this.thread_number = thread_number;
        this.barrier = barrier;
        this.numBucketss = numBuckets;
        this.numThreads = numThreads;
        this.willLoop = false;
        this.bucketss = new ArrayList<LinkedHashSet<Vertex>>(numBucketss);
        for(int i=0; i<numBuckets; i++){
          bucketss.add(new LinkedHashSet<Vertex>());
        }
        bucketss.get(0).add(vertices[0]);
      }
      ...
      public void run(){
        for(;;){
          for(;;){
            ...
            Boolean threadSending = false;

            long thisThreadID = Thread.currentThread().getId();
            int thisThread = activeThreads.get(thisThreadID);

            isFinished.set(thisThread,false);
            while(bucketss.get(index).size() > 0){
              requests = findRequests(bucketss.get(index),true);
              removed.addAll(bucketss.get(index));
              bucketss.set(index, new LinkedHashSet<Vertex>());

              for(Request requestt : requests){
                //System.out.println("thread num " + thread_number + " req "+ requests.size());
                if(vertexThreads.get(requestt.v) == thisThreadID){
                  try{
                    requestt.relax(bucketss);
                  }
                  catch (Coordinator.KilledException e) {}
                }
                else{
                  long threadId = vertexThreads.get(requestt.v);
                  if(activeThreads.get(threadId) != null){
                    int thread = activeThreads.get(threadId);
                    requestQueue.get(thread).add(requestt);
                  }
                  isThreadSending.set(thisThread, true);
                  threadSending = true;
                }
              }
            }
            if(!threadSending){
              isThreadSending.set(thisThread, false);
            }
            if(!willLoop){
              bAwait();
            }

            while(!requestQueue.get(thisThread).isEmpty()){
              Request req = requestQueue.get(thisThread).poll();
              try{
                req.relax(bucketss);
              }
              catch (Coordinator.KilledException e) {}
            ...
            if(bucketss.get(index).size()==0){
              isFinished.set(thisThread, true);
            }
            // int bak=index;
            // while(bucketss.get(index).size()!=0&&index<=numBucketss){
            //   index++;
            // }
            // if(index<nextIndex){
            //   nextIndex=index;
          ...
          willLoop=false;
          // if(nextIndex!=1000&&index>nextIndex){
          //   index=nextIndex;
          // }
          index++;
          //nextIndex = n;
          if(index>=numBucketss){
            break;
          }
        }
        return;
